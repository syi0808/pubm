...
describe("run", () => {
  ...
  describe("normal mode (publishOnly=false)", () => {
    ...
    it("creates a task list with all expected tasks", async () => {
      ...
      expect(tasks[3].title).toBe("Bumping version");
      expect(tasks[4].title).toBe("Publishing");
  ...
  describe("task skip flags", () => {
    ...
    it("does not skip dry-run publish regardless of options", async () => {
      const options = createOptions({ skipPublish: true, preview: true });
      await run(options);

      const callArgs = mockedCreateListr.mock.calls[0];
      const tasks = callArgs[0] as any[];

      expect(tasks[2].skip).toBeUndefined();
    });
  ...
  describe("multi-package publishing", () => {
    it("uses per-package registries from packages config in normal mode", async () => {
      ...
      const callArgs = mockedCreateListr.mock.calls[0];
      const tasks = callArgs[0] as any[];
      const publishTask = tasks[4]; // Publishing task
