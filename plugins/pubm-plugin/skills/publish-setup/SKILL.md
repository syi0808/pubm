...
- When suggesting npm scripts, use `"release": "pubm --phase prepare"` if CI is configured, or `"release": "pubm"` if it is not. Always use `"release:ci": "pubm --phase publish"` for CI.
- In CI, use `--phase publish` for publish plus GitHub Release, or `--phase publish` for publish only.
