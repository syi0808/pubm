...
Split release jobs should run exactly one phase: `--phase prepare` before the CI handoff, or `--phase publish` inside CI.
...
- **Split release jobs require a phase flag.** Use `pubm --phase prepare` for the prepare job or `pubm --phase publish` for the CI publish job.
