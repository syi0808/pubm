...
The asset pipeline runs after the build step and before GitHub Release upload. Plugins can intercept every stage through six hooks that mirror the pipeline's internal structure.
...
Returns the final upload filename (without extension — the extension is appended from `compressFormat`). Use this hook when the name template system is not expressive enough.
