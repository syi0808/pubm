...
Use [Release Workflows](/pubm/guides/release-workflows/) to choose between Direct Release and Split CI Release.

This page focuses on the CI half of Split CI Release: running `pubm --phase publish` after `pubm --phase prepare` has already written manifest versions, created tags, and pushed the release commit and tags.
...
Use the [`syi0808/pubm/actions/changeset-check`](https://github.com/syi0808/pubm/tree/main/actions/changeset-check) GitHub Action to enforce that every PR includes a changeset. `pubm init` generates this workflow automatically when changesets are enabled.
