...
Run `pubm init` to interactively generate this configuration file. The wizard only creates a config file when your choices differ from the defaults shown below.
...
These fields are part of the public schema, but they are not yet active toggles across every publish path. Treat them as typed config surface, not as guaranteed runtime switches.
...
If the package has `package.json` and `jsr.json`, registries are inferred automatically. No config file is needed — `pubm.config.ts` is optional. Only create it when you need to override a default (custom branch, plugins, release assets, etc.).
