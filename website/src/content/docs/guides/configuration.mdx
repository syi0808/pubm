...
`pubm` is CLI-first by design, but real projects quickly outgrow flag-only configuration. `pubm.config.*` is where you keep repository-level publish targets, package layout, plugins, and release planning rules.
...
Whether created GitHub Releases should be drafts. Use the `--skip-release` CLI flag when a run should skip GitHub Release creation entirely.
...
Glob patterns for packages that should be versioned and published but excluded from git tag creation and GitHub Release drafts. Applies only when `release.versioning.mode` is `"independent"`.
...
`phase` is runtime state, not project config. Select Split CI Release phases with `pubm --phase prepare` or `pubm --phase publish`; omit `--phase` for Direct Release.
...
export default defineConfig({
  ...
  packages: [{ path: "packages/web" }, { path: "packages/server" }],
});
```
...
export default defineConfig({
  ...
  packages: [{ path: "packages/sdk" }, { path: "crates/sdk-core" }],
});
```
...
- See the [Changesets](/pubm/guides/changesets/) guide for more on changesets and conventional commit recommendations.
