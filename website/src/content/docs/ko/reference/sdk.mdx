...
| `phase` | `"prepare" \| "publish"` | Split CI Release 단계 하나를 실행합니다. 생략하면 Direct Release를 실행합니다. |
...
| `ReleaseContext` | type | `afterRelease` 훅에 전달되는 release 메타데이터입니다. `displayLabel`, `version`, `tag`, `releaseUrl`, `assets`를 포함합니다. |
