...
`@pubm/plugin-brew`는 CLI 프로젝트가 각 릴리스와 함께 Homebrew formula를 동기화하는 데 도움을 줍니다. PR 생성 후 파이프라인이 실패하면 생성된 Homebrew PR을 닫는 롤백 액션을 등록합니다.
...
`@pubm/plugin-brew`는 필요한 자격 증명과 check를 자체적으로 선언합니다. 별도로 수동 설정할 필요는 없습니다.
...
로컬 실행에서는 pubm이 이 토큰을 프롬프트하고 시스템 keyring에 저장합니다. CI에서는 `PUBM_BREW_GITHUB_TOKEN` 환경 변수로 토큰을 제공합니다. `pubm --mode ci --phase prepare` 또는 `pubm secrets sync`를 실행하면 저장된 토큰을 저장소의 GitHub Secrets로 동기화해 CI workflow가 자동으로 사용할 수 있습니다.
...
`brewCore`는 `homebrew/homebrew-core`를 포크하고, formula를 업데이트한 뒤, PR을 자동으로 엽니다. `brewTap`과 마찬가지로 `PUBM_BREW_GITHUB_TOKEN` 자격 증명과 preflight check도 자동으로 선언합니다.
