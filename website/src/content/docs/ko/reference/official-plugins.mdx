...
로컬 실행에서는 pubm이 이 토큰을 프롬프트하고 시스템 keyring에 저장합니다. CI에서는 `PUBM_BREW_GITHUB_TOKEN` 환경 변수로 제공합니다. `pubm --phase prepare` 또는 `pubm secrets sync`를 실행하면 저장된 토큰을 저장소의 GitHub Secrets로 동기화해 CI workflow가 바로 사용할 수 있게 됩니다.
