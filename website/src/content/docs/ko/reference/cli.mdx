...
| `--phase <phase>` | - | 릴리스 단계입니다. `prepare`는 검증, 버전 변경, 태그, push를 실행하고 `publish`는 manifest 버전을 publish하고 release를 생성합니다. 생략하면 둘 다 실행합니다. |
...
- 분리된 워크플로의 남은 단계가 비대화형으로 동작하도록 전환합니다.
...
Publish 단계는 manifest 버전을 읽고 registry publish와 GitHub Release 생성을 API로 실행합니다. versioning과 태깅이 분리된 워크플로의 앞 단계에서 이미 끝났을 때 사용합니다. `--release-draft`를 넘기면 published release 대신 draft release를 만듭니다.
