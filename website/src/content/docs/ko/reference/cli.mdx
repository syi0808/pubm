---
title: CLI 레퍼런스
description: pubm CLI의 명령, 플래그, 릴리스 흐름입니다.
---
...
Publish 단계는 최신 Git tag에서 publish하고 GitHub Release를 API로 생성합니다. version bump와 태깅이 파이프라인의 앞 단계에서 이미 끝났을 때 사용합니다. `--release-draft`를 넘기면 published release 대신 draft release를 만듭니다.
