...
이 레퍼런스는 명령 형태별로 정리되어 있습니다.

- 최상위 publish 명령: `pubm [version]`
- `init`, `secrets`, `sync` 같은 관리 명령
- `pubm changesets ...` 아래의 changeset 워크플로 명령

일상적인 수동 사용에서 가장 중요한 명령은 단순히 다음입니다.

```bash
pubm
```

버전 인자 없이 `pubm`을 실행하면 대화형의 정석 경로가 됩니다. 릴리스 파이프라인이 계속되기 전에 터미널에서 다음 버전을 고를 수 있습니다.
...
| `pubm init` | 대화형 설정 마법사 — 패키지 감지, 설정, changeset, CI 워크플로, 코딩 에이전트 스킬을 처리합니다. |
...
버전을 생략하면, `pubm`은 대화형 터미널에서 다음 버전을 물을 수 있습니다.
...
| `--snapshot [tag]` | — | 임시 snapshot 버전을 publish합니다(기본 태그: `snapshot`). |
...
`pubm`을 위한 대화형 설정 마법사입니다. TTY(대화형 터미널)가 필요합니다.

```bash
pubm init
```

마법사는 다음 단계를 순서대로 진행합니다.

1. **패키지 감지** — 저장소에서 매니페스트 파일(`package.json`, `jsr.json`, `Cargo.toml`)과 워크스페이스 설정을 스캔한 뒤, 감지된 패키지와 레지스트리를 확인합니다.
...
5. **코딩 에이전트 스킬** — Claude Code, Codex CLI, Gemini CLI용 코딩 에이전트 스킬 다운로드 및 설치를 제안합니다(`pubm setup-skills`와 동일).
