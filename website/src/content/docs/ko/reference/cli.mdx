...
| `pubm init` | `pubm.config.ts`와 `.pubm/changesets/`를 생성합니다. |
| `pubm init --changesets` | PR changeset 검사 워크플로도 설정합니다. |
...
- `.pubm/changesets/`에 대한 `.gitignore` 업데이트

검토된 changeset 파일을 모든 product change에 포함해야 하는 저장소에서 사용합니다.
