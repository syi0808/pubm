...
`pubm`은 파일 경로에서 OS, 아키텍처, ABI, variant를 파싱하는 내장 플랫폼 감지 엔진을 제공합니다. 이것은 자동 asset 이름 지정, OS-aware 압축 기본값, 그리고 `@pubm/plugin-brew` 같은 플러그인이 소비하는 구조화된 `ParsedPlatform` 객체를 뒷받침합니다.
