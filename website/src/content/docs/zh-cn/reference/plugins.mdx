...
- `credentials` - 返回这个插件需要的 token 列表。pubm 会在初始化时调用一次，用来提示缺失的 token、将其注入环境变量，并在执行 `--ci-prepare` 时同步到 GitHub Secrets。
- `checks` - 返回与核心 prerequisite/condition 阶段一起运行的 preflight checks。每个 check 通过 `phase` 字段声明自己的执行时机。
...
- `ctx.runtime.pluginTokens`：以 `PluginCredential.key` 为键的已解析插件凭证值映射。它会在凭证收集步骤之后填充，并可在后续所有 hooks 中使用。
