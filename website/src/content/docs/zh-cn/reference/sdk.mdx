...
| `phase` | `"prepare" \| "publish"` | 运行一个 Split CI Release 阶段。省略则运行 Direct Release。 |
...
| `PluginTaskContext` | type | 传给 `PluginCheck.task` 的 runner 无关包装器，提供 `output`、`title` 和 `prompt()`。 |
...
| `ReleaseContext` | type | 传给 `afterRelease` hooks 的 release 元数据。包含 `displayLabel`、`version`、`tag`、`releaseUrl` 和 `assets`。 |
...
const plugin: PubmPlugin = {
  name: "announce",
  hooks: {
    afterRelease(_ctx, releaseCtx) {
      console.log("published", releaseCtx.displayLabel, releaseCtx.version);
    },
  },
};
