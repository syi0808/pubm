...
| `pubm init` | 创建 `pubm.config.ts` 和 `.pubm/changesets/`。 |
| `pubm init --changesets` | 同时设置 PR 的 changeset 检查工作流。 |
...
- 针对 `.pubm/changesets/` 的 `.gitignore` 更新

当仓库中的每个产品变更都应该包含一个经过审阅的 changeset 文件时，使用这个选项。
