...
| `--phase <phase>` | - | Release 阶段：`prepare`（验证、改版本、打 tag 并 push）或 `publish`（发布 manifest 版本并创建 release）。省略则两者都运行。 |
...
Publish 阶段会读取 manifest 版本、发布到 registries，并通过 API 创建 GitHub Release。适合版本更新和打 tag 已经在拆分 workflow 的前一步完成的场景。传入 `--release-draft` 可以创建草稿 release，而不是正式发布。
