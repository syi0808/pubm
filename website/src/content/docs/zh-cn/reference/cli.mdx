...
4. **工作流 Setup**: 提供启用 changesets 工作流的选项（创建 `.pubm/changesets/`、`.github/workflows/pubm-changeset-check.yml` 并更新 `.gitignore`），以及生成 CI 发布工作流（`.github/workflows/pubm-release-pr.yml` and `.github/workflows/pubm-publish.yml`）。
...
`pubm changesets version` analyzes pending changesets and conventional commits together. Changesets win when both sources cover the same package. See the [Configuration guide](/pubm/zh-cn/guides/configuration/#release-analysis) for `release.changesets` and `release.commits` options.
