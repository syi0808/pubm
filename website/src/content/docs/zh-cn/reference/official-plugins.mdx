...
本地运行时，pubm 会提示输入这个 token，并把它存到系统 keyring。CI 中则通过 `PUBM_BREW_GITHUB_TOKEN` 环境变量提供。运行 `pubm --phase prepare` 或 `pubm secrets sync`，可以把已保存的 token 同步到仓库的 GitHub Secrets，供 CI workflow 自动使用。
