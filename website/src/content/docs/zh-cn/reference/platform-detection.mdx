...
`pubm` 内置了一个平台检测引擎，用来从文件路径中解析 OS、architecture、ABI 和 variant。这为自动资产命名、按 OS 决定的压缩默认值，以及插件（例如 `@pubm/plugin-brew`）使用的结构化 `ParsedPlatform` 对象提供支持。
...
**在路径模式中捕获变量。** 如果模式包含 `{os}`、`{arch}`、`{abi}`、`{variant}` 或 `{platform}`，`pubm` 会直接从这些位置提取值：
...
**自动 segment 扫描。** 当没有使用 capture 变量时，`pubm` 会先按 `/` 把路径拆成 segments，再按 `-` 拆分每个 segment。然后每个 token 会按顺序在 OS、arch、ABI、variant 和 vendor 表中测试。每个类别第一个匹配项获胜。
...
vendor、ABI 和 variant 会刻意排除在 `{platform}` 之外，以保持常见情况简短且易读。它们可以通过 `{vendor}`、`{abi}` 和 `{variant}` 单独访问。
...
这个对象会附加到流经流水线的每个资产上，并在 `afterRelease` hook 中通过 `ReleaseAsset.platform` 暴露给插件。

## OS 表

canonical 值会用于模板变量和插件比较。解析路径时会识别别名。
