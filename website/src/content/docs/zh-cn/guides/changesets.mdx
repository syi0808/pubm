---
title: Changesets
description: 记录发布意图、生成 changelog，并控制版本变更。
---

`pubm` 将 changeset 作为“下一次应该发布什么”的持久记录。它不会根据提交信息或分支名推断版本，而是读取 `.pubm/changesets/` 中已提交的文件，并将它们转换为版本变更和 changelog 条目。

## 为什么 changesets 重要

- 每个 pull request 中都能保留可审阅的发布计划
- 跨包的版本 bump 计算是确定性的
- changelog 内容在发布日前就能写好，而不是发布后补写

## 文件位置

待处理的 changeset 位于：

```text
.pubm/changesets/
```

运行交互式 setup 向导来初始化该目录：

```bash
pubm init
```

在 setup 过程中，`pubm init` 会询问是否启用 changesets 工作流。选择"是"会创建 `.pubm/changesets/` 目录，生成 `.github/workflows/pubm-changeset-check.yml` 用于 PR 强制检查，并更新 `.gitignore` 以跟踪 changeset 文件，同时忽略其他 `.pubm/` 内容。

## 创建 changeset

```bash
pubm changesets add
```

在 monorepo 中，`pubm` 还会询问受影响的包，以及每个包应该获得什么 bump 类型。

非交互式用法：

```bash
pubm changesets add \
  --packages @acme/core,@acme/react \
  --bump minor \
  --message "Add token-based auth to the public client."
```

## changeset 文件包含什么

```md
---
"@acme/core": minor
"@acme/react": patch
---

Add token-based auth to the public client.
```

元数据控制版本计算，正文会成为 changelog 摘要。

## 查看待处理变更

在消耗 changeset 之前，先检查它们：

```bash
pubm changesets status
pubm changesets status --verbose
pubm changesets version --dry-run
```

## changeset 如何被消耗

当你用 `pubm` 运行正常发布流程时，待处理的 changeset 会在版本步骤中被消耗。

该流程会：

1. 读取待处理的 `.pubm/changesets/*.md` 文件
2. 计算最终的包版本
3. 更新 manifest 版本
4. 生成 changelog 条目
5. 删除已消耗的 changeset 文件
6. 创建发布提交和 tags

如果你只想提前执行版本计算部分，也可以运行：

```bash
pubm changesets version
```

这个命令适合编辑流程或受控流程，但在 `pubm` 或 `pubm --phase publish` 之前并不是必需的。

## 生成 changelog

如果你想先生成 changelog 文本，而不消耗版本：

```bash
pubm changesets changelog --dry-run
pubm changesets changelog --version 1.8.0
```

## Conventional commit recommendations

`pubm changesets version` analyzes conventional commits alongside pending changeset files. If a package has no changeset recommendation, a releasable conventional commit can create the bump recommendation. Non-conventional commits, ignored commit types, and commits that cannot be mapped to a package are treated as unversioned changes; they do not create bumps by themselves.

### Default commit type mapping

| Commit type | Bump |
| --- | --- |
| `feat` | `minor` |
| `fix` | `patch` |
| `perf` | `patch` |

A `BREAKING CHANGE` footer or a `!` suffix (e.g. `feat!:`) always produces a `major` bump.

### Configuring custom mappings

Override the default mapping in `pubm.config.ts`:

```ts
export default defineConfig({
  release: {
    commits: {
      types: {
        feat: "minor",
        fix: "patch",
        perf: "patch",
        docs: "patch",
      },
    },
  },
});
```

Release PR automation warns by default when only unversioned changes are found and no versioned release PR can be opened. Configure that with `release.pullRequest.unversionedChanges`: `"warn"` (default), `"ignore"`, or `"fail"`.

## 从 `.changeset/` 迁移

```bash
pubm changesets migrate
```

这会把 `.changeset/` 中的待处理文件迁移到 `.pubm/`。
