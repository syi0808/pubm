...
| `--phase <phase>` | - | Phase de release : `prepare` (valider, versionner, taguer et pousser) ou `publish` (publier les versions des manifests et créer les releases). Omettez-le pour exécuter les deux. |
...
- passe en comportement non interactif pour le reste du workflow séparé
...
La phase de publication lit les versions des manifests, publie les registries et crée une GitHub Release via l'API. Utilisez-la quand la mise à jour de version et le tagging ont déjà eu lieu plus tôt dans le workflow séparé. Passez `--release-draft` pour créer un draft au lieu d'une release publiée.
