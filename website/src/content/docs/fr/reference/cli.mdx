...
`pubm changesets version` analyzes pending changesets and conventional commits together. Changesets win when both sources cover the same package. See the [Configuration guide](/pubm/fr/guides/configuration/#release-analysis) for `release.changesets` and `release.commits` options.
