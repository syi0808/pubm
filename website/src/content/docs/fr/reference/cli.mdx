...
| `pubm init` | Créer `pubm.config.ts` et `.pubm/changesets/`. |
| `pubm init --changesets` | Mettre aussi en place le workflow de vérification des changesets en pull request. |
...
Initialiser le dépôt pour `pubm`.
...
- des mises à jour `.gitignore` pour `.pubm/changesets/`

Utilisez cela sur les dépôts où chaque changement de produit doit inclure un fichier changeset relu.
