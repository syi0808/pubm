...
- `credentials` - renvoie la liste des tokens dont ce plugin a besoin. pubm l'appelle une fois à l'initialisation pour demander les tokens manquants, les injecter dans l'environnement et les synchroniser vers GitHub Secrets lors d'un `--phase prepare`.
...
- `ctx.options` - options CLI résolues (version, tag, phase, dryRun, etc.)
...
| `ghSecretName` | `string` | Nom du GitHub Secret qui doit contenir ce token lors de la synchro `--phase prepare`. |
...
Wrapper agnostique au runner passé en second argument à `PluginCheck.task`.
...
- `prompt()` - lance une invite interactive pubm.
...
const notifyPlugin = {
  ...
  hooks: {
    async afterRelease(_ctx, releaseCtx) {
      console.log(
        `Publié ${releaseCtx.displayLabel} ${releaseCtx.version} (${releaseCtx.tag})`,
      );
    },
  },
};
