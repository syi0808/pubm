...
En local, pubm demande ce token et le stocke dans le keyring du système. En CI, passez-le via la variable d'environnement `PUBM_BREW_GITHUB_TOKEN`. Exécutez `pubm --phase prepare` ou `pubm secrets sync` pour synchroniser le token stocké vers les GitHub Secrets du dépôt et le rendre disponible aux workflows CI.
