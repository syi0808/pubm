...
| `phase` | `"prepare" \| "publish"` | Exécuter une phase de Split CI Release. Omettre pour Direct Release. |
...
| `PluginCredential` | type | Descripteur déclaratif d'un token ou secret requis par un plugin. Voir la [Référence de l'API des plugins](/pubm/fr/reference/plugins/#plugincredential). |
| `PluginCheck` | type | Descripteur d'un check de preflight apporté par un plugin. Voir la [Référence de l'API des plugins](/pubm/fr/reference/plugins/#plugincheck). |
| `PluginTaskContext` | type | Wrapper agnostique au runner passé à `PluginCheck.task`. Expose `output`, `title` et `prompt()`. |
...
| `ReleaseContext` | type | Métadonnées de release transmises aux hooks `afterRelease`. Inclut `displayLabel`, `version`, `tag`, `releaseUrl` et `assets`. |
