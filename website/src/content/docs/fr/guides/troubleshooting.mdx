...
`pubm --phase prepare` est une vraie préparation Split CI Release : elle peut écrire les versions, créer des tags et pousser le commit de release et les tags. Utilisez-la seulement lorsque vous voulez préparer une publication CI, pas comme simple vérification sans effet de bord.
...
- exécutez `pubm --phase prepare` localement
...
- distinguez le flux de publication normal du `--phase publish`

`pubm` crée une GitHub Release via l'API. Sans `GITHUB_TOKEN` en mode local, vous pouvez saisir un token, ouvrir un draft dans le navigateur ou passer l'étape. `--phase publish` crée une GitHub Release via l'API dans le cadre du pipeline.
