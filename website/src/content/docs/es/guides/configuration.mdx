...
Si el paquete tiene `package.json` y `jsr.json`, los registries se infieren automáticamente. No hace falta un archivo de configuración: `pubm.config.ts` es opcional. Créalo solo cuando necesites sobrescribir un valor por defecto, como un branch personalizado, plugins o assets de release.
