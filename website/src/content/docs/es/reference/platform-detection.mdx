...
`pubm` incluye un motor integrado de detección de plataformas que analiza OS, arquitectura, ABI y variant a partir de rutas de archivo. Esto impulsa el nombrado automático de assets, los valores por defecto de compresión según el sistema operativo y el objeto estructurado `ParsedPlatform` que consumen plugins como `@pubm/plugin-brew`.
...
**Escaneo automático de segmentos.** Cuando no se usan variables de captura, `pubm` divide la ruta en segmentos con `/` y luego cada segmento en `-`. Cada token resultante se prueba contra las tablas de OS, arch, ABI, variant y vendor en ese orden. El primer match de cada categoría gana.
...
Vendor, ABI y variant se excluyen intencionalmente de `{platform}` para mantener el caso común corto y legible. Se pueden acceder individualmente mediante `{vendor}`, `{abi}` y `{variant}`.
