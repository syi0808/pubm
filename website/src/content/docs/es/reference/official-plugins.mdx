...
Los plugins oficiales se mantienen junto con `pubm` y siguen las mismas expectativas de release y compatibilidad que el proyecto central. Úsalos cuando quieras una integración bien soportada sin tener que construir tu propio plugin desde cero.

## `@pubm/plugin-brew`

`@pubm/plugin-brew` ayuda a los proyectos CLI a mantener las fórmulas de Homebrew sincronizadas con cada release. Registra acciones de rollback para cerrar cualquier PR de Homebrew creada si el pipeline falla tras la creación del PR.
...
En ejecuciones locales, pubm pide este token y lo guarda en el keyring del sistema. En CI, proporciona el token mediante la variable de entorno `PUBM_BREW_GITHUB_TOKEN`. Ejecuta `pubm --mode ci --phase prepare` o `pubm secrets sync` para sincronizar el token guardado con los GitHub Secrets de tu repositorio, de modo que los workflows de CI puedan usarlo automáticamente.
