...
| `phase` | `"prepare" \| "publish"` | Ejecuta una fase de Split CI Release. Omítelo para Direct Release. |
...
| `PluginTaskContext` | type | Wrapper agnóstico del runner que se pasa a `PluginCheck.task`. Expone `output`, `title` y `prompt()`. |
...
| `ReleaseContext` | type | Metadatos de release pasados a los hooks `afterRelease`. Incluye `displayLabel`, `version`, `tag`, `releaseUrl` y `assets`. |
