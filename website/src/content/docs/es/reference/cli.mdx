...
Esta referencia está organizada por forma de comando:
...
Para el uso diario por personas, el comando más importante es simplemente:

```bash
pubm
```

Ejecutar `pubm` sin argumento de versión es el flujo interactivo ideal. Te permite elegir la siguiente versión en la terminal antes de que continúe el pipeline de release.
...
| `pubm init` | Asistente de configuración interactivo: detección de paquetes, configuración, changesets, workflows de CI y skills de agentes de código. |
...
Si omites la versión, `pubm` puede pedir la siguiente versión en una terminal interactiva.
...
| `--snapshot [tag]` | — | Publica una versión snapshot temporal (tag por defecto: `snapshot`). |
...
Asistente de configuración interactivo para `pubm`. Requiere un TTY (terminal interactiva).

```bash
pubm init
```

El asistente recorre cada etapa en orden:

1. **Detección de paquetes** — escanea el repositorio en busca de archivos manifest (`package.json`, `jsr.json`, `Cargo.toml`) y la configuración del workspace, y confirma los paquetes y registries detectados.
...
5. **Skills de agentes de código** — ofrece descargar e instalar skills de agentes de código para Claude Code, Codex CLI o Gemini CLI (igual que `pubm setup-skills`).
