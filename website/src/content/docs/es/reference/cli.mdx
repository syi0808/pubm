...
| `--phase <phase>` | - | Fase de release: `prepare` (validar, versionar, etiquetar y hacer push) o `publish` (publicar versiones de manifests y crear releases). Omítelo para ejecutar ambas. |
...
- cambia a comportamiento no interactivo para el resto del workflow dividido
...
La fase de publicación lee las versiones de los manifests, publica en los registries y crea un GitHub Release mediante la API. Úsala cuando el versionado y el tagging ya hayan ocurrido antes en el workflow dividido. Pasa `--release-draft` para crear un release en borrador en lugar de uno publicado.
