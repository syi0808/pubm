...
| `pubm init` | Crea `pubm.config.ts` y `.pubm/changesets/`. |
| `pubm init --changesets` | También configura el workflow de comprobación de changesets en PR. |
...
Inicializa el repositorio para `pubm`.
...
- actualizaciones de `.gitignore` para `.pubm/changesets/`

Usa esto en repositorios donde cada cambio de producto deba incluir un archivo de changeset revisado.
