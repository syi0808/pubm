...
| `phase` | `"prepare" \| "publish"` | Fuehrt eine Split CI Release-Phase aus. Weglassen fuer Direct Release. |
...
| `ReleaseContext` | type | Release-Metadaten für `afterRelease`-Hooks. Enthält `displayLabel`, `version`, `tag`, `releaseUrl` und `assets`. |
