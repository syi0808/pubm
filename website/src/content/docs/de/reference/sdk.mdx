...
| `dryRun` | `boolean` | Zeigt den Task-Plan an, ohne Git-Zustand zu aendern oder zu veroeffentlichen. |
| `mode` | `"local" \| "ci"` | Ausfuehrungsmodus: `local` (interaktiv) oder `ci` (nicht interaktiv, tag-basiert). |
| `prepare` | `boolean` | Fuehrt nur die Vorbereitungsphase aus (Validierung und Publish-Checks im Dry-Run). |
| `publish` | `boolean` | Fuehrt nur die Publish-Phase aus (Veroeffentlichung vom neuesten Tag). |
| `releaseDraft` | `boolean` | Erstellt einen Draft-GitHub-Release statt eines veroeffentlichten Releases. |
| `branch` | `string` | Verlangt einen bestimmten Branch vor dem Veröffentlichen. |
| `anyBranch` | `boolean` | Deaktiviert die Branch-Pruefung fuer einen Lauf. |
| `tag` | `string` | Veroeffentlicht unter einem Dist-Tag wie `latest`, `next` oder `beta`. |
| `snapshot` | `string \| boolean` | Baut eine Snapshot-Version anhand der konfigurierten Vorlage. |
| `packages` | `PackageConfig[]` | Ueberschreibt automatisch gefundene Pakete. |

Fuer Config-getriebene Releases laedt `pubm()` automatisch `pubm.config.*`, loest die Paket-Erkennung auf und verbindet die Plugins, bevor der Task-Graph ausgefuehrt wird.

## Config-Helfer

Diese Helfer sind die wichtigsten Einstiegsstellen fuer Code, der `pubm.config.*` lesen oder validieren will.

| Export | Art | Zweck |
|------|-----|-------|
| `defineConfig` | function | Typisierter Helfer zum Schreiben von `pubm.config.ts`. |
| `loadConfig` | function | Laedt `pubm.config.*` von der Festplatte und gibt `PubmConfig \| null` zurueck. |
...
| `ResolvedPubmConfig` | type | Vollstaendig aufgeloeste Config, die `resolveConfig()` zurueckgibt. |
| `PackageConfig` | type | Paketbezogener Config-Eintrag fuer Discovery und Publish-Planung. |
...
`PackageConfig` ist der Ort, an dem du pro Paket Registries, Ecosystem, Build-Command oder Test-Command ueberschreiben kannst:
...
| `PluginCommand` / `PluginSubcommand` / `PluginCommandOption` | type | Modell fuer benutzerdefinierte CLI-Befehle von Plugins. |
| `PluginRunner` | class | Hilfsklasse, die Hooks ausfuehrt und Registries/Ecosystems aus geladenen Plugins sammelt. |
| `ReleaseContext` | type | Release-Metadaten fuer `afterRelease`-Hooks. Enthält `packageName`, `version`, `tag`, `releaseUrl` und `assets`. |
...
| `PreparedAsset` | type | Asset nach Komprimierung und Hashing, bereit fuer den Upload. |
...
Diese Exporte treiben die Release-Asset-Pipeline an und sind fuer Plugin-Autoren und eigenes Tooling nuetzlich.
...
| `runAssetPipeline` | function | Fuehrt die komplette Asset-Pipeline (resolve, transform, compress, name, hash, checksums) fuer eine aufgeloeste Config und ein Hook-Set aus. |
...
Siehe [Release Assets](/pubm/de/de/guides/release-assets/) und [Asset Pipeline Hooks](/pubm/de/de/guides/asset-pipeline-hooks/) fuer die Anwendungsleitfaeden.
...
| `ChangelogEntry` / `DependencyUpdate` | Eingaben fuer das Rendern des Changelogs. |
| `PackageVersionInfo` | Paketname, Version und Pfad, die von Discovery-Helfern zurueckgegeben werden. |
...
| `buildDependencyGraph` | function | Baut einen internen Abhaengigkeitsgraphen aus Paketmetadaten. |
| `topologicalSort` | function | Sortiert Graphknoten so, dass Abhaengigkeiten zuerst kommen. |
...
| `Git` | class | Schlanke Huelle um Git-Befehle, die fuer Release-Automatisierung genutzt werden. |
...
| `DiscoverOptions` | Eingabe fuer `discoverPackages()`. |
| `DiscoveredPackage` | Automatisch erkanntes Paket mit Pfad, Registries und Ecosystem. |
| `PackageNode` | Eingabeknoten fuer den Aufbau des Abhaengigkeitsgraphen. |
...
Das Paket-Root exportiert ausserdem die folgenden Helfer fuer Tooling-Integrationen.

| Export | Art | Zweck |
|------|-----|-------|
| `validateEntryPoints` | function | Prueft `main`, `module`, `types`, `exports` und `bin`-Pfade in einem Paket-Manifest. |
| `detectExtraneousFiles` | function | Markiert Dateien, die normalerweise nicht in Publish-Artefakten landen sollten. |
| `getPackageJson` | function | Liest das naechste `package.json`, optional mit `jsr.json`-Fallback. |
| `replaceVersion` / `replaceVersionAtPath` | functions | Schreibt Paketversionen in Manifests um. |
| `version` | function | Liest die aktuelle Paketversion aus `package.json` oder `jsr.json`. |
...
| `syncGhSecrets` | function | Schiebt Registry-Tokens ueber `gh secret set` in GitHub Secrets. |
...
| `notifyNewVersion` | function | Prueft, ob es eine neuere veroeffentlichte `pubm`-Version gibt, und gibt ein Update-Banner aus. |
| `consoleError` | function | Formatiert und gibt `pubm`-Fehler aus. |
| `PUBM_VERSION` / `PUBM_ENGINES` | constants | Paketmetadaten, die zur Laufzeit verfuegbar sind. |

Wichtige zugehoerige Typen:

| Typ | Zweck |
|-----|-------|
| `EntryPointError` | Eintrag fuer Validierungsergebnisse bei defekten Manifest-Pfaden. |
| `ExtraneousFile` | Eintrag fuer Validierungsergebnisse bei Dateien, die nicht veroeffentlicht werden sollten. |
| `SnapshotOptions` | Eingabe fuer `generateSnapshotVersion()`. |
| `Runtime` | `"node" \| "bun"`. |
