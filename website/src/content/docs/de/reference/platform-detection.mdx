...
description: Eingebaute Tabellen fuer OS, Architektur, ABI und Varianten, die fuer die automatische Plattform-Analyse verwendet werden.
---

`pubm` bringt eine eingebaute Plattform-Erkennungs-Engine mit, die OS, Architektur, ABI und Varianten aus Dateipfaden analysiert. Das bildet die Grundlage fuer automatische Asset-Benennung, OS-abhängige Standardwerte bei der Komprimierung und das strukturierte `ParsedPlatform`-Objekt, das von Plugins wie `@pubm/plugin-brew` verwendet wird.
...
**Automatisches Scannen von Segmenten.** Wenn keine Capture-Variablen verwendet werden, zerlegt `pubm` den Pfad zuerst an `/` in Segmente und dann jedes Segment an `-`. Jedes daraus entstehende Token wird der Reihe nach gegen die Tabellen fuer OS, Architektur, ABI, Variante und Vendor getestet. Der erste Treffer pro Kategorie gewinnt.
...
- wenn `{platform}` als Capture-Variable verwendet wurde: das erfasste Segment unveraendert (`darwin-arm64`)
...
Vendor, ABI und Variante werden absichtlich aus `{platform}` ausgeschlossen, damit der haeufige Fall kurz und lesbar bleibt. Sie sind einzeln ueber `{vendor}`, `{abi}` und `{variant}` erreichbar.
...
Dieses Objekt wird jedem Asset angehaengt, das durch die Pipeline laeuft, und ueber `ReleaseAsset.platform` im `afterRelease`-Hook sichtbar.

## OS-Tabelle

Kanonische Werte werden in Template-Variablen und Plugin-Vergleichen verwendet. Aliase werden waehrend des Pfad-Parsens erkannt.
...
ABI-Tokens tauchen in Rust-Zieltripletts und Toolchain-Namen fuer Cross-Compilation auf.

| Value | Meaning |
|-------|---------|
| `gnu` / `glibc` | GNU C Library |
| `musl` | musl libc (gut fuer statische Builds) |
...
| `uclibc` | uClibc fuer eingebettete Targets |
...
Vendor-Tokens werden waehrend des Parsens erkannt, aber nicht in die `{platform}`-Variable aufgenommen. Du erreichst sie ueber `{vendor}`.

| Value | Meaning |
|-------|---------|
| `unknown` | Generisch (Rust-Standard fuer Linux-Targets) |
...
- [Release Assets](/pubm/de/de/guides/release-assets/) - Konfigurationsleitfaden
- [Asset Pipeline Hooks](/pubm/de/de/guides/asset-pipeline-hooks/) - Referenz fuer Plugin-Hooks
