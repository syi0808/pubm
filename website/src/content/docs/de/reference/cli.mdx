...
| `--phase <phase>` | - | Release-Phase: `prepare` (validieren, versionieren, taggen und pushen) oder `publish` (Manifest-Versionen veröffentlichen und Releases erstellen). Weglassen, um beide auszuführen. |
...
- schaltet für den Rest des geteilten Workflows auf nicht interaktives Verhalten um
...
Die Publish-Phase liest Manifest-Versionen, veröffentlicht in Registries und erstellt einen GitHub Release über die API. Verwende sie, wenn Versionierung und Tagging vorher schon im geteilten Workflow passiert sind. Mit `--release-draft` erstellst du statt eines veröffentlichten Releases einen Draft.
