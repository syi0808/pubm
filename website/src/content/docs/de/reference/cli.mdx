...
Für den taeglichen Gebrauch ist der wichtigste Befehl einfach:

```bash
pubm
```

Wenn du `pubm` ohne Versionsargument aufrufst, landest du im interaktiven Standardpfad. Dort kannst du im Terminal die nächste Version waehlen, bevor die Release-Pipeline weiterlaeuft.
...
Wenn du die Version weglässt, kann `pubm` in einem interaktiven Terminal nach der nächsten Version fragen.
...
Interaktiver Setup-Wizard für `pubm`. Erfordert ein TTY (interaktives Terminal).
...
5. **Coding-Agent-Skills** — bietet an, Coding-Agent-Skills für Claude Code, Codex CLI oder Gemini CLI herunterzuladen und zu installieren (entspricht `pubm setup-skills`).
