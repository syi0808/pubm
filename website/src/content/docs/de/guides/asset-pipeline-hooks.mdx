...
Die Asset-Pipeline laeuft nach dem Build-Schritt und vor dem Upload in den GitHub Release. Plugins koennen jede Stufe ueber sechs Hooks abfangen, die die interne Struktur der Pipeline spiegeln.
...
releaseAssets config
  ...
  → [nameAsset]      — die Name-Vorlage mit dynamischer Logik ueberschreiben
  → sha256 hash
  → [generateChecksums] — Pruefsummen-Dateien an die Asset-Liste anhaengen
  → GitHub-Release-Upload (integriert)
  → [uploadAssets]   — an zusaetzliche Ziele hochladen (S3, R2, CDN usw.)
...
Alle Hooks werden zu `PluginHooks` hinzugefuegt und ueber das normale `plugins`-Array in `pubm.config.ts` registriert.
...
Wird einmal mit der vollstaendigen Liste der per Glob gefundenen Assets aufgerufen. Verwende diesen Hook, um Assets zu filtern, programmgesteuert gefundene Dateien hinzuzufuegen oder Metadaten anzuhängen.
...
const filterPlugin: PubmPlugin = {
  ...
  hooks: {
    resolveAssets(resolved, ctx) {
      // Nur Assets fuer unterstuetzte Plattformen aufnehmen
...
`TransformedAsset` erweitert `ResolvedAsset` um das optionale Feld `extraFiles?: string[]`. Dateien aus `extraFiles` werden waehrend der Komprimierung zusammen mit der Hauptdatei ins Archiv gepackt.
...
Gib ein Array zurueck, um aus einem Eingang mehrere Assets zu erzeugen. Jedes Element durchlaeuft den Rest der Pipeline unabhaengig.
...
Ersetzt die eingebaute Komprimierungslogik vollstaendig. Wenn gesetzt, ist der Hook dafuer verantwortlich, ein `CompressedAsset` mit dem richtigen `filePath` und `compressFormat` zu erzeugen.

**Input:** `TransformedAsset`
**Output:** `CompressedAsset`

Verwende das, wenn du ein Format brauchst, das `pubm` nicht nativ unterstuetzt, oder wenn du zusaetzliche Dateien in das Archiv packen willst.
...
const customArchivePlugin: PubmPlugin = {
  ...
  hooks: {
    async compressAsset(asset, ctx) {
      ...
      // Fuer andere Plattformen auf den Standard zurueckfallen
...
Gibt den finalen Upload-Dateinamen zurueck, ohne Erweiterung. Die Erweiterung wird aus `compressFormat` angehaengt. Verwende diesen Hook, wenn das Name-Vorlagensystem nicht ausdrucksstark genug ist.
...
Wird mit der vollstaendigen Liste vorbereiteter Assets aufgerufen. Verwende diesen Hook, um eine Pruefsummen-Manifestdatei anzuhaengen.

**Input:** `PreparedAsset[]` - alle Assets inklusive ihrer `sha256`-Werte
**Output:** `PreparedAsset[]` - urspruengliche Assets plus zusaetzliche Pruefsummen-Dateien
...
| `nameAsset` | Letzter gewinnt - jedes Plugin erhaelt dieselben `(asset, ctx)`-Argumente; der Rueckgabewert des zuletzt registrierten Plugins wird verwendet |
| `generateChecksums` | Kette - die Ausgabe von Plugin N ist der Input von Plugin N+1 |
| `uploadAssets` | Additiv - jedes Plugin erhaelt die urspruengliche `PreparedAsset[]`-Liste unabhaengig; alle Ergebnisse werden zusammengefuehrt |

Jeder nicht abgefangene Fehler in einer Hook-Kette bricht die Pipeline ab und leitet an den `onError`-Hook weiter.

## Vollstaendiges Beispiel: Code-Signing + Pruefsummen + S3
...
- [Release Assets](/pubm/de/de/guides/release-assets/) - deklarative Konfigurationsreferenz
- [Platform Detection](/pubm/de/de/reference/platform-detection/) - OS- und Architektur-Tabellen
- [Plugins API Reference](/pubm/de/de/reference/plugins/) - vollstaendige Plugin-Schnittstelle
