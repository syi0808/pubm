...
Wenn das Paket `package.json` und `jsr.json` hat, werden Registries automatisch abgeleitet. Eine Config-Datei ist nicht nötig - `pubm.config.ts` ist optional. Erstelle sie nur, wenn du einen Standardwert überschreiben musst (z. B. eigener Branch, Plugins, Release-Artefakte usw.).
