...
Plugins koennen auch eigene benoetigte Credentials deklarieren. pubm sammelt und löst diese automatisch auf. Bei `--phase prepare` fragt es nach fehlenden Werten und synchronisiert sie mit GitHub Secrets. Bei `--phase publish` liest es die aufgeloesten Werte nur noch aus der Umgebung und bricht mit einem Fehler ab, wenn ein erforderliches Credential fehlt.
