...
- `credentials` — returns the list of tokens this plugin needs. Called once during initialization so pubm can prompt for missing tokens, inject them into the environment, and sync them to GitHub Secrets when running `--ci-prepare`.
- `checks` — returns preflight checks that run alongside the core prerequisite and condition phases. Each check declares when it should run via its `phase` field.
...
- `output` — display a status line beneath the check title in the task list UI.
