...
- `ctx.runtime.rollback`: rollback tracker used by plugins to register cleanup actions.

`ctx.options.phase` is the runtime Split CI Release phase selected by `pubm --phase prepare` or `pubm --phase publish`. Plugins can inspect it when hook behavior must differ by phase, but workflow selection guidance belongs in [Release Workflows](/pubm/guides/release-workflows/).
