---
title: CLI Reference
description: Commands, flags, and release flows for the pubm CLI.
---
...
Publish phase publishes from the latest Git tag and creates a GitHub Release via API. Use it when the version bump and tagging already happened earlier in the pipeline. Pass `--release-draft` to create a draft release instead of a published one.
