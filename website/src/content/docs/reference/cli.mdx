...
| `--phase <phase>` | - | Release phase: `prepare` (validate, version, tag, and push) or `publish` (publish manifest versions and create releases). Omit to run both. |
...
- switches into non-interactive behavior for the remaining split workflow
...
Publish phase reads manifest versions, publishes registries, and creates a GitHub Release via API. Use it when versioning and tagging already happened earlier in the split workflow. Pass `--release-draft` to create a draft release instead of a published one.
