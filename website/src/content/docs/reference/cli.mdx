...
Interactive setup wizard for `pubm`. Requires a TTY (interactive terminal).

```bash
pubm init
```

The wizard walks through each stage in order:

1. **Package Detection** — scans the repository for manifest files (`package.json`, `jsr.json`, `Cargo.toml`) and workspace configuration, then confirms the detected packages and registries.
...
5. **Coding Agent Skills** — offers to download and install coding agent skills for Claude Code, Codex CLI, or Gemini CLI (same as `pubm setup-skills`).
