...
`pubm` ships a built-in platform detection engine that parses OS, architecture, ABI, and variant from file paths. This powers automatic asset naming, OS-aware compression defaults, and the structured `ParsedPlatform` object consumed by plugins such as `@pubm/plugin-brew`.
...
**Capture variables in the path pattern.** If the pattern contains `{os}`, `{arch}`, `{abi}`, `{variant}`, or `{platform}`, `pubm` extracts values from those positions directly:
...
**Automatic segment scanning.** When no capture variables are used, `pubm` splits the path into segments on `/`, then splits each segment on `-`. Each resulting token is tested against the OS, arch, ABI, variant, and vendor tables in order. The first match per category wins.
...
Vendor, ABI, and variant are intentionally excluded from `{platform}` to keep the common case short and readable. They are accessible individually via `{vendor}`, `{abi}`, and `{variant}`.
