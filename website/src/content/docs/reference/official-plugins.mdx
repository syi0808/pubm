...
`@pubm/plugin-brew` helps CLI projects keep Homebrew formulae in sync with each release. Registers rollback actions to close any Homebrew PRs created if the pipeline fails after PR creation.
...
This plugin keeps non-manifest files aligned with your package version. Registers rollback actions to restore original file contents if the pipeline fails after version sync.
