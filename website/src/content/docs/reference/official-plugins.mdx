...
- update `url` and `sha256` values for release assets
...
That command scans for likely version references and prints candidate `regex` or `json` targets you can copy into plugin config.
