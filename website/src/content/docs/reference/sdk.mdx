...
| `preview` | `boolean` | Show the task plan without mutating Git state or publishing. |
...
| `preflight` | `boolean` | Simulate CI locally and dry-run publish checks. |
| `ci` | `boolean` | Publish from the latest tag and create a GitHub Release. |
| `publishOnly` | `boolean` | Publish artifacts without rerunning the full preparation pipeline. |
