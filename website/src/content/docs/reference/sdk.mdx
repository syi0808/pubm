...
| `PluginHooks` | type | Lifecycle hooks such as `beforeBuild`, `afterPublish`, `onError`, and `afterRelease`. |
...
| `ReleaseContext` / `ReleaseAsset` | types | Metadata passed to `afterRelease` hooks. |
