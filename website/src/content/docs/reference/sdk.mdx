...
| `phase`        | `"prepare" \| "publish"` | Run one Split CI Release phase. Omit for Direct Release.                          |
...
`phase` is runtime workflow state on `Options`, not a `pubm.config.*` field. Omit it for Direct Release, or pass `"prepare"` / `"publish"` for Split CI Release. See [Release Workflows](/pubm/guides/release-workflows/) for operator-facing behavior.
...
| `PluginTaskContext`                                          | type     | Runner-agnostic wrapper passed to `PluginCheck.task`. Exposes `output`, `title`, and `prompt()`.                                                                                                                                 |
...
| `ReleaseContext`                                             | type     | Release metadata passed to `afterRelease` hooks. Includes `displayLabel`, `version`, `tag`, `releaseUrl`, and `assets`.                                                                                                         |
