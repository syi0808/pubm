...
console.log("[@pubm/darwin-arm64] Compiling...");

await $`bun install @napi-rs/keyring-darwin-arm64@1.2.0 --os='darwin' --cpu='arm64'`;
...
if (!result.success) {
  ...
  console.error("[@pubm/darwin-arm64] Build failed");
  process.exit(1);
}
...
console.log(`[@pubm/darwin-arm64] Done → ${OUT_FILE}`);
