...
console.log("[@pubm/linux-arm64-musl] Compiling...");

await $`bun install @napi-rs/keyring-linux-arm64-musl@1.2.0 --os='linux' --cpu='arm64'`;
...
const result = await Bun.build({
  ...
  plugins: [
    createKeyringPlugin(
      ROOT,
      "./node_modules/@napi-rs/keyring-linux-arm64-musl/keyring.linux-arm64-musl.node",
    ),
  ],
  define,
});
...
if (!result.success) {
  ...
  console.error("[@pubm/linux-arm64-musl] Build failed");
  process.exit(1);
}

console.log(`[@pubm/linux-arm64-musl] Done → ${OUT_FILE}`);
