...
console.log("[@pubm/linux-x64-musl-baseline] Compiling...");

await $`bun install @napi-rs/keyring-linux-x64-musl@1.2.0 --os='linux' --cpu='x64'`;
...
if (!result.success) {
  ...
  console.error("[@pubm/linux-x64-musl-baseline] Build failed");
  process.exit(1);
}

console.log(`[@pubm/linux-x64-musl-baseline] Done → ${OUT_FILE}`);
